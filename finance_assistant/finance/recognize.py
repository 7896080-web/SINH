...
def _payment_schema(categories: list[str]) -> dict:
    ...
    return {
        ...
        "required": ["is_payment", "direction", "amount", "currency", "date", "card_last4",
                     ...
                     "looks_personal", "from_business_account", "to_owner"],
...
- card_last4: последние 4 цифры карты ИЛИ счёта списания («Карта списания •1234», \
...
- to_owner: true, если получатель — сам владелец (то же ФИО, что у ИП-плательщика, \
«перевод себе», «между своими счетами»). Перевод другому человеку — false.
