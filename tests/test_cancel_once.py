...
Отмена перемещений в 1С НЕ идемпотентна: обработка ищет движения заказа по
шаблону, а обратный документ сам под этот шаблон подходит, поэтому повторная
отмена создаёт ФАНТОМНЫЙ ПРИХОД товара, которого не было. Починить это в самой
обработке нельзя — база 1С только боевая, тестовой нет.
