...
def test_scheduler_hourly_jobs_fire_right_after_start(web_db):
    ...
    откладывал их на час.

    Прежняя редакция этого теста требовала, чтобы сверка шла через несколько минут
    ПОСЛЕ запроса выгрузки — «с запасом на ответ 1С». Этот запас и оказался причиной
    находки pm40: он молча закладывался на то, что 1С ответит быстрее запаса, а
    обработка на боевом запускается своим расписанием, раз в 10 минут, и отвечала
    через семь. Сверка смотрела в папку раньше ответа, уходила, и следующий часовой
    цикл отбраковывал тот же файл как устаревший — причём фаза держалась до
    перезапуска воркера, то есть сверка не срабатывала вообще.

    Поэтому фиксированного окна ожидания больше нет: сверка ходит смотреть папку
    часто (интервал закреплён в `test_reconciliation_silence.py`), и ждать ответ
    одним-единственным взглядом ей не нужно. Здесь остаётся то, что от этого не
    изменилось: после деплоя оба задания берутся за дело сразу, а не через час."""
    ...
    # сверка — тоже сразу, а не через час: снимок мог прийти и до рестарта
    assert (reconciliation.next_run_time - now).total_seconds() <= 300
