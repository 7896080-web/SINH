...
@repository_only
def test_the_scrap_line_format_agrees_between_the_two_sides():
    """Обработка разбирает строку ИНДЕКСАМИ, и сдвиг на одно поле молчалив.

    Хоз. операция, приехавшая в ответственного, означала бы списание не на ту
    статью — при этом `Поля[8]` заполнено, отказа нет, документ проведён. Ловить
    такое по результату пришлось бы в отчётах через месяц, поэтому сверяем
    номера полей: слева — что кладёт `ftp_channel`, справа — что читает модуль.
    """
    text = LIVE.read_text(encoding="utf-8-sig")
    block = text[text.index('ИначеЕсли Команда = "SCRAP_RETURN"'):]
    block = block[:block.index("ИначеЕсли Команда = \"CANCEL_MOVEMENT\"")]

    # Порядок полей в задании: …7=дата, 8=хоз. операция, 9=ответственный.
    assert "ДатаДок = ?(Поля.Количество() > 7, Поля[7]" in block
    assert "ХозОперация = ?(Поля.Количество() > 8, СокрЛП(Поля[8])" in block
    assert "Ответственный = ?(Поля.Количество() > 9, СокрЛП(Поля[9])" in block


@repository_only
def test_the_scrap_document_fills_every_required_field():
    """Реквизиты, без которых документ списания либо не проведётся, либо
    проведётся не туда. Проверка грубая — по наличию присваивания, — но ловит
    ровно то, что теряется при правке: строку, удалённую заодно с соседней.
    """
    text = LIVE.read_text(encoding="utf-8-sig")
    block = text[text.index("Функция СписатьВозврат"):]
    block = block[:block.index("КонецФункции")]

    for assignment in ("Документ.Склад =", "Документ.Магазин =",
