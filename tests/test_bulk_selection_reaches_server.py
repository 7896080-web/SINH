...
# ------------------------------------- отбор шире экрана — об этом надо спросить

def test_an_edit_reaching_offscreen_rows_asks_first():
    """24.09 на бою, в первый же день после наката: в отборе накопилось 846
    строк от прежних фильтров, оператор нашёл поиском ОДИН товар, отметил его и
    нажал «Расчёт» — задание ушло по всем 846.

    Это обратная сторона правки, которая двумя коммитами раньше научила форму
    дописывать невидимые отметки. Само свойство верное: без него уезжало 500
    строк из 1739, и оператор об этом не узнавал. Но с экрана случай выглядел
    как работа с одной строкой — искал один товар, видел один товар, отметил
    один товар, — и счётчик «846 шт» этого не объяснял: он маленький, серый и
    не говорит, что 845 строк сейчас не видно.

    Значит спрашиваем ровно тогда, когда экран расходится с тем, что уедет.
    """
    page = PAGE.read_text(encoding="utf-8")

    handler = re.search(r'getElementById\("bulk-form"\)\.addEventListener\("click".+?\n    \}\);',
                        page, re.S)
    assert handler is not None, "обработчик кнопок массовой правки исчез"
    body = handler.group(1) if handler.groups() else handler.group(0)

    assert "offscreen" in body, "разница «отмечено минус видно» больше не считается"
    # Отказ проверяем ВНУТРИ этой ветки, а не по всему обработчику: рядом стоит
    # второе подтверждение (у действий, отправляющих ноль) со своим
    # preventDefault, и проверка «есть где-то в теле» мимо него не промахнётся —
    # мутация «убрать отказ из ветки про экран» проходила незамеченной.
    branch = re.search(r"if \(offscreen > 0 && !confirm\(.+?\n            \}", body, re.S)
    assert branch is not None, "ветка про строки вне экрана исчезла"
    assert "e.preventDefault()" in branch.group(0), (
        "вопрос без отказа — это не подтверждение, а уведомление")
    # Вопрос обязан называть ЧИСЛА: без них «часть строк не видна» не говорит
    # человеку ничего — ровно та же причина, по которой их требует подтверждение
    # на действиях, отправляющих ноль.
    assert "selected.size" in body and "shown" in body


def test_the_question_is_asked_for_every_bulk_button():
    """Ловушка общая, а не про «Расчёт»: любая кнопка массовой правки уедет по
    тому же отбору. Обработчик поэтому ловит `button[name="action"]`, а не
    только те кнопки, у которых есть свой `data-confirm`."""
    page = PAGE.read_text(encoding="utf-8")

    assert 'closest(\'button[name="action"]\')' in page, (
        "вопрос снова задаётся только части кнопок")


def test_a_selection_that_fits_the_screen_asks_nothing():
    """Лишний вопрос на безопасном действии приучает жать «Да» не читая — и
    тогда вопрос на опасном тоже перестаёт работать. Поэтому условие строгое:
    спрашиваем, только если отмеченного БОЛЬШЕ, чем показано."""
    page = PAGE.read_text(encoding="utf-8")

    assert "offscreen > 0 &&" in page, "вопрос задаётся и когда расхождения нет"
    # «Весь отбор» молчит намеренно: галочка стоит в шапке той самой таблицы,
    # счётчик называет число текущего отбора, и строка под таблицей говорит это
    # словами.
    assert re.search(r"if \(!whole\) \{\s*\n\s*var shown = 0;", page), (
        "«весь отбор» больше не исключён из проверки")
