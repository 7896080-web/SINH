...
Строка ниже оставлена для машин, где git есть.
...
Copy-Item C:\sync_admin\sync_admin.db C:\sync_admin\sync_admin.db.before-restore
Copy-Item C:\sync_admin\backups\sync_admin-ГГГГММДД-ЧЧММСС.db C:\sync_admin\sync_admin.db
nssm start sync_admin_worker
nssm start sync_admin_web
```

Текущую базу перед заменой отложите в сторону, а не удаляйте: в ней заказы,
принятые после копии, и их придётся сверять с площадками руками.
