...
COMMON_FIELDS = [
    ...
    Field("WEB_HEARTBEAT_URL", "Пинг сторожа веб-службы",
          placeholder="https://hc-ping.com/...",
          hint="ВТОРОЙ чек, отдельный от первого. Воркер дёргает его, только если "
               "веб-служба ответила у себя на машине. Молчат оба — умер воркер; "
