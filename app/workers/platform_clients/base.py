...
class PlatformClient(ABC):
    ...
    # Умеет ли ЭТОТ клиент читать остатки обратно. Признаком, а не по ответу:
    # `get_stocks` отдаёт `None` и когда метод не реализован, и когда площадка
    # промолчала, — а это разные вещи. Первое штатно и навсегда (у Ozon и Kit
    # метод не написан: вслепую такое не пишется), второе означает, что сверка
    # по кабинету не прошла, и об этом надо сказать. Без признака отличить их
    # по одному счётчику `skipped` нельзя, и снятая ручка чтения или хронический
    # 429 у WB выглядели бы ровно как штатная тишина Ozon.
    reads_stocks = False
