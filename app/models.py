...
class Product(Base):
    ...
    # Порог, который надо удержать, когда 1С ответит на новую дату. Ставится
    # при сдвиге даты НАЗАД, если снимка на неё ещё нет: подобрать факт не от
    # чего, а к моменту ответа отличить «поехали назад» от «поехали вперёд»
    # будет уже неоткуда. Снимается при первом же применении. NULL — обычный
    # случай, удерживать нечего. См. `offset_base.pin_offset`.
