...
EXPECTED_INTERVAL_SECONDS = {
    ...
    "verify_stock": 1800 * 3,
    # Суточные задания. Без своей записи они судились бы по умолчанию в десять
    # минут и держали бы /health красным все оставшиеся 23 часа 50 минут — ровно
    # это и случилось 21.09, когда бэкап и чистку добавили, а сюда их записать
    # забыли. Тот же дефект уже был у недельного импорта справочника и у
    ...
    "alerts": 300 * 3,
    # Сторож «мёртвого человека» — тоже раз в пять минут. Он единственный, чьё
    # молчание замечает НЕ эта система, а внешний сервис; но и она сама обязана
    # видеть, что задание ходит, иначе о его пропаже узнаешь от сторожа —
    # то есть тревогой, которой не было.
    "watchdog": 300 * 3,
}
...
REQUIRED_WORKERS = {
    ...
    "alerts": 1200,
    # Сторож просит первый прогон через полминуты после старта — ждать его
    # двадцать минут, как уведомления, здесь незачем: у него нет причины
    # откладываться.
    "watchdog": 300,
}
