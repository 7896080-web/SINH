...
def apply_retention(db: Session) -> dict:
    ...
    # ТОЛЬКО терминальные статусы. Запись в `pending` — это незаконченная
    # отправка: остаток у нас уже списан, а на площадку не уехал. Удалить её
    # значит потерять единственный след того, что площадка продаёт по старому
    # числу. Сколько бы лет ей ни было.
    stats["dispatch_queue"] = _purge(
        db, DispatchQueueItem,
        (DispatchQueueItem.created_at < now - DISPATCH_KEEP)
        & DispatchQueueItem.status.in_([DispatchStatus.sent, DispatchStatus.error]),
        "очередь рассылки")
