...
def offset_from_base(product: Product | None) -> int | None:
    ...
    где расхождение — `учёт 1С − сколько лежит на самом деле`, ХРАНИМОЕ свойство
    товара (`Product.stock_discrepancy`). До 23.09 оно нигде не хранилось и
    выводилось из пары «остаток на дату − факт на дату», то есть носителем была
    ДАТА: сменили её — факт стёрся, расхождение исчезло, порог схлопнулся до
    брони, и наружу поехал остаток, завышенный ровно на расхождение. Прежняя
    формула — частный случай нынешней:

        (остаток ЦС на дату − факт) + бронь  =  расхождение + бронь
    ...
    if product is None:
        return None
    if product.stock_discrepancy is not None:
        # Основной путь: расхождение измерено и ХРАНИТСЯ на товаре, дата к нему
        # отношения не имеет. Бронь по-прежнему двигает порог ровно на себя —
        # это и есть смысл формулы: сначала не отдаём то, чего на складе нет,
        # потом не отдаём то, что держим у себя.
        return product.stock_discrepancy + (product.reserve or 0)
    if product.offset_base_date is None or product.offset_base_stock is None:
        return None
    # Расхождение не измеряли (`stock_discrepancy is NULL`) — считаем по-старому.
    # Факт при этом либо не введён, либо равен учёту: ввод факта, ОТЛИЧНОГО от
    # учёта, расхождение записывает, и до этой ветки дело уже не доходит. То есть
    # здесь всегда выходит ровно бронь, и ветка нужна ради товаров, настроенных
    # до появления колонки, — их порог не должен дрогнуть.
...
def recompute_offset(product: Product | None) -> bool:
    ...
    Единственный, кто пишет `broadcast_offset` по формуле. Вызывать обязательно
    при смене ЛЮБОГО из чисел, из которых он складывается, включая бронь: иначе
    новая бронь останется словами, потому что в пороге будет сидеть старая.

    **Про дату здесь больше НЕТ ни слова, и это не упущение.** Раньше тут стояло
    «даты нет — порог не трогаем», и для прежней модели это было верно: порог
    выводился из пары «остаток на дату − факт на дату», то есть без даты его
    не из чего было считать. С хранимым расхождением носителем стал ТОВАР:
    `offset_from_base` отдаёт число всякий раз, когда расхождение измерено, — и
    дату можно двигать куда угодно или снять вовсе, порог не шелохнётся. Ровно
    ради этого механизм и переписан.

    Молчим мы теперь в одном случае: расхождение не измеряли И даты нет. Там
    действительно живёт число, введённое руками, и стирать его нельзя.

    Комментарий, утверждающий обратное тому, что делает функция, дороже
    отсутствующего: в этом коде правила передаются объяснением рядом с кодом, а
    не тестом.
    """
