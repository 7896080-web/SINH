...
# Веб-служба живёт на этой машине и отвечает у себя; наружу порт не открыт, и
# внешним монитором её не достать. Адрес совпадает с тем, по которому накат
# ждёт `/health` (`deploy/update_windows.ps1`) и который ставит установщик.
WEB_PROBE_URL = "http://127.0.0.1:8000/health"


def ping_web_alive(db: Session) -> str:
    """Подтвердить внешнему сторожу, что ВЕБ-СЛУЖБА отвечает. Пустая строка — ок.

    Закрывает единственную дыру, оставшуюся после `ping_alive`. Разберём по
    случаям, что вообще видно изнутри:

      * легли обе службы — воркер молчит, первый сторож будит. Покрыто;
      * умер воркер, веб жив — то же самое, и это самый дорогой случай. Покрыто;
      * **умер веб, воркер жив — не видно НИЧЕМ.**

    Третий случай выглядит изнутри полностью исправным: воркер пингует сторожа,
    ...
    продолжают синхронизироваться, денег никто не теряет; слепнет ЧЕЛОВЕК: ни
    отчёта, ни «Диагностики», ни включения трансляции, ни разбора зависшего
    задания 1С. И скверный частный случай: настоящая критичная тревога в это
    время уйдёт в Telegram со словами «разберите на Диагностике», а Диагностика
    не откроется.

    Полярность та же, что у `ping_alive`, и СМЫСЛ У КАЖДОГО СИГНАЛА ОДИН — это
    и есть причина второго чека, а не экономии ради одного. Подмешай мы «и веб
    отвечает» в первый пинг, его молчание стало бы значить «что-то не так»,
    неотличимо от падения воркера. Раздельно таблица однозначна:

      * молчат ОБА → умер воркер или машина;
      * молчит только этот → умерла веб-служба;
      * молчит только первый → невозможно по построению (этот пинг идёт ИЗНУТРИ
        воркера), и если такое увидели — сломан сам механизм.

    Не пингуем, если веб не ответил: в этом всё и дело. Пинг здесь — не
    «сообщить о беде», а «подтвердить, что её нет», и подтверждать нечего.
    """
    url = _env(db, "WEB_HEARTBEAT_URL")
    if not url:
        return ""
    try:
        requests.get(WEB_PROBE_URL, timeout=TIMEOUT).raise_for_status()
    except Exception as e:                           # noqa: BLE001
        # Ровно тот случай, ради которого всё написано: молчим НАМЕРЕННО, о
        # беде расскажет внешний сервис. Но в оговорку это идёт обязательно —
        # иначе единственным следом осталась бы тишина, которую не с чем
        # сопоставить.
        logger.warning("веб-служба не отвечает (%s): %s", WEB_PROBE_URL, e)
        return f"веб-служба не отвечает: {type(e).__name__}: {e}"[:200]
    try:
        requests.get(url, timeout=TIMEOUT).raise_for_status()
        return ""
    except Exception as e:                           # noqa: BLE001
        # Веб жив, а сказать об этом не вышло. Молчание сторожа прочтётся как
        # «веб умер» — то есть тревога будет ложной, и это надо видеть.
        logger.warning("сторож веб-службы не отвечает (%s): %s", url, e)
        return f"сторож веб-службы не ответил: {type(e).__name__}: {e}"[:200]
